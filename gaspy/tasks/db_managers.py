...
class UpdateAtomsCollection(luigi.Task):
    '''
    This task will dump all of the completed FireWorks into our `atoms` Mongo
    ...
    Args:
        n_processes     An integer indicating how many threads you want to use
                        ...
                        argument.
        seed            A string that is used as a tag to verify whether this
    ...
    def run(self):
        ...
        # Save the seed so that we know that this task is done (for now)
        save_task_output(self, self.seed)
    ...
    def make_doc_from_fwid(self, fwid):
        '''
        ...
        doc = self.__patch_old_document(doc, atoms, fw)
        return doc
    ...
    def __patch_old_document(self, doc, atoms, fw):
        '''
        ...
        patched_atoms = self.__patch_atoms_from_old_vasp(atoms, fw)
        for key, value in make_doc_from_atoms(patched_atoms).items():
            patched_doc[key] = value

        # Guess some VASP setttings we never recorded
        patched_doc['fwname']['vasp_settings'] = self.__get_patched_vasp_settings(fw)

        # Some of our old FireWorks had string-formatted Miller indices. Let's
        # fix those.
        if 'miller' in fw.name:
            patched_doc['fwname']['miller'] = self.__get_patched_miller(fw.name['miller'])

        return patched_doc

    def __patch_atoms_from_old_vasp(self, atoms, fw):
        '''
        ...
        if any(constraint.todict()['name'] != 'FixAtoms' for constraint in atoms.constraints):
            if fw.created_on < datetime(2018, 12, 1):
                ...
                patched_atoms = self.__get_final_atoms_object_with_vasp_forces(launch_id)
                return patched_atoms

        # If the FireWork is new or simple, then no patching is necessary
        return atoms

    def __get_final_atoms_object_with_vasp_forces(self, launch_id):
        '''
        ...
        fw_launch_file = (read_rc('fireworks_info.backup_directory') +
                          '/%d.tar.gz' % launch_id)
        temp_loc = self.__dump_file_to_tmp(fw_launch_file)
